...
import { ImageRecognitionService, type ProductRecognitionResult, type ReceiptData } from '../services/ImageRecognitionService';
  ...
  const handleImageUpload = async (file: File) => {
    ...
    try {
      if (mode === 'product') {
        ...
        const result = await ImageRecognitionService.recognizeProductFromImage(file);
        ...
        const expiryDate = await ImageRecognitionService.extractExpiryDate(file);
        if (expiryDate) {
          ...
          setError('Nie udało się rozpoznać daty ważności na obrazie');
        }
      }
    } catch (error) {
